...
- I tried to add Redaction in one go, and...
  - It's possible there was an issue with the rewritten findOverlapsTier (because of how it handles 0-row `overlapsBounds`)
  - I also had to rewrite getTimes() so it wouldn't exclude Redaction, but I don't think it worked
...
To come up with a to-do list of sorts, I'm saving the diff file with coloring for Notepad++ by running (in the `apls` branch)

```bash
# Thanks, https://stackoverflow.com/a/28307437/5432121!
git diff HEAD..fix-fixoverlaps > fixO.diff
```

I then modify the file to delete changes once I'm happy with them.
...
(Later, do this in other spots where I'm mapping over names.)
...
Okay, now I've rewritten findOTier args to be better imap-able.
Does it still work?
...
times %>% 
  pluck(1) %>% 
  imap(~ which(names(.x)==.y))
```

Oh, I see why it doesn't work for imap.
Okay, use the current somewhat janky but workable solution


Wait, actually, why don't I just map over timesEAF _and_ include timesEAF as an arg?
Oh, short answer: I'd still have to awkwardly partition it so the rest of the timesTier elements still could be clumped together, so just passing the name is the most straightforward way to do it.
Wait, or I could imap over both timesTier & each name!

Great---no changes.

## Add "Only proceed if there are any overlaps" (0-row `overlapBounds`) to `findOverlapsTier()`

I _think_ this is working, but surprisingly, it's still showing overlaps-to-fix even when I run the file thru the file checker and it supposedly reduces overlaps to 0.
Some of that is the fact that I didn't actually implement multiple rounds of fixing.

But even after running the file through, there are 2 overlaps per tier still showing.
This is just one overlap per tier: a boundary shared by two side-by-side annotations.
Oh, and because the snapping-to-boundary is simultaneous r/t sequential, they swap places every time.
So that'll be fixed soon.

I figured this out by adding this to `overlapsIssues()`:

```{r, eval=FALSE}
ol <- times %>% 
  imap(~ imap(.x, findOverlapsTier, timesEAF=.x))

return(ol)
```

And this to debugPrint:

```{r, eval=FALSE}
list(
  overlapsIssues1 = overlapsIssues(eaflist(), tierInfo()) %>% 
    map_depth(2, ~ select(.x, 1:7)),
  overlapsIssues1 = overlapsIssues(eaflist(), tierInfo()) %>% 
    map_depth(2, ~ select(.x, 8:14))
)
```


In the meantime, I need to know if this works!
Modify `step3/HD06-1_OverlapsResolved.eaf` so it _actually_ has all overlaps resolved

Okay, testing it now, I don't get any problems if there are no overlaps in the input.

Finally, add the monitor switch to overlapsIssues.

That updated the OverlapsResolved.eaf results and All results, but nothing else. Great!

## Modify `getTimes()` to use `tiers` arg r/t `df`

- `tiers`: character vector of tier names to include (default: `NULL`, for "all tiers")
- `df`: `tierInfo()`, always including only `spkrTier==TRUE`

So I have to move the identification of spkrTiers out to `overlapsIssues()`, which calls `getTimes()` directly.
Check that this little bit works:

```{r}
list(a = c("HD06", "Interviewer HD06"), 
     b = c("CB01", "Interviewer CB01")) %>% 
  map(append, "Redaction")
```

Actually, I can just move this out to a separate function that overlapsIssues calls once (and also gets called in output$export)

```{r}
library(jsonlite)
tierInfo <- fromJSON("../APLS Elan File Checker/tests/shinytest/test-step3-expected/All.json") %>% 
  pluck("export", "tierInfo")
tierInfo %>% 
  filter(SpkrTier) %>% 
  nest(data = -File) %>% 
...
Re-running test-step3...no changes.


## Add "Ensure tierOrder matches overlapsInit names" to `fixOverlaps()`

What if I just drop it in?

Oh, I have to hard-code `tierOrder`, but it seems to work (with `fixOverlapRedact` both T & F).

Ran test-step3, no changes.


## Add "Fix each tier in turn" to `fixOverlaps()`, **which will change test-step3 results**

Hmm, I'm realizing I need to move more stuff from `overlapsIssues()` to `fixOverlaps()`.
Like, since I need to rerun `getTimes()` after each round, `fixOverlaps()` needs `tierInfo()` so it can run `getOverlapTiers()`.

As with last week, I'm really struggling to find the source of "no applicable method for 'xml_find_all' applied to an object of class "NULL""

Try it interactively

```{r}
eaflist <- 
  list(`HD06-1_GoodOnly.eaf` = "../APLS Elan File Checker/tests/shinytest/step3/HD06-1_GoodOnly.eaf") %>% 
  map(read_xml)
tierInfo <- fromJSON("../APLS Elan File Checker/tests/shinytest/test-step3-expected/HD06-1_GoodOnly.eaf.json") %>% 
  pluck("export", "tierInfo")
tierNames <- getOverlapTiers(tierInfo, FALSE)
eafName <- names(eaflist)[1]
tierNames ; eafName
```


```{r}
getTimes(eaflist %>% pluck(eafName), eafName, tierNames %>% pluck(eafName))
```

Oh, I was confusing eaf & eaflist for the millionth time. ARGH.

Anyway, it looks like it works now.

Rerun test-step3; results should change.
They do, and the output files themselves have no overlaps. Wahoo!

And, as predicted, IntFirst has different results, since it's taken the in-file tier order as the hierarchy:

```{r}
dir("../APLS Elan File Checker/tests/shinytest/test-step3-expected/", "Good.+eaf$", full.names=TRUE) %>% 
  set_names(basename(.)) %>% 
  map(read_lines) %>% 
  map(str_subset, "\"ts1[23]\"")
```



## Dynamically determine `tierOrder`
...
Okay great!
Add this to `getOverlapTiers()`

```{r}
getOverlapTiers(tierInfo, TRUE)
getOverlapTiers(tierInfo, FALSE)
```

Excellent! If all goes well, step3 should change GoodOnly_IntFirst but not GoodOnly.
Indeed it does!

```{r}
dir("../APLS Elan File Checker/tests/shinytest/test-step3-current/", "Good.+eaf$", full.names=TRUE) %>% 
  set_names(basename(.)) %>% 
  map(read_lines) %>% 
  map(str_subset, "\"ts1[23]\"")
```
...
## Add Redaction to overlap finding & fixing

If I've set this up right, it's as easy as setting `fixOverlapRedact <- TRUE`.

It seems to work, sorta, but it looks like it's really just widening out the annotations.
I guess that's because in a situation where (e.g.) the entire HD06 annotation is off the Redactions annotation by (say) +50ms, the HD06 left boundary will overlap with Redaction, and the Redaction right boundary will overlap with HD06, but the 'outer' boundaries won't.
So only the 'inner' boundaries get snapped.
I think that's fine.

Re-run the test!

Oh shoot, I'm getting the error: "names(overlapsInit): HD06Interviewer HD06. tierOrder: RedactionHD06Interviewer HD06".
That's for the files w/ no Redaction annotations.

So, if there's an empty tier, `getTimesTier()` makes it NULL, which means it doesn't get returned by `getTimes()`.
It doesn't work to just keep the NULLs, bec that messes up stg else.
So just overwrite: `tierNamesFile <- names(overlapsInit)`.
I checked, and `names(overlapsInit)` does retain the preferred order.


# That's enough fixing for now.

Time to close out this scratchpad.
